...
async function main() {
  ...
  const adminUser = await prisma.user.upsert({
    where: { email: 'admin@play123.com' },
    update: {},
    create: {
      name: 'Super Admin',
      email: 'admin@play123.com',
  ...
  const developerUser = await prisma.user.upsert({
    where: { email: 'developer@play123.com' },
    update: {},
    create: {
      name: 'Apex Game Developer',
      email: 'developer@play123.com',
  ...
  console.log('✅ PostgreSQL database seeded successfully with Om Nom Run & games!');
}
